...
class DnnModel:
    ...
    def get_bigru_model(self):
        ...
        model_dnn.add(Bidirectional(CuDNNGRU(self.nodes, return_sequences=True), input_shape=self.input_shape,activation=self.activation))
    ...
    def get_bilstm_model(self):
        model_dnn = Sequential()
        model_dnn.add(Bidirectional(CuDNNLSTM(self.nodes, return_sequences=True), input_shape=self.input_shape,
    ...
    def get_gru_model(self):
        model_dnn = Sequential()
        model_dnn.add(Bidirectional(CuDNNGRU(self.nodes, return_sequences=True), input_shape=self.input_shape,
                                    activation=self.activation))
        model_dnn.add(Dropout(self.dropout))
    ...
    def test(self, X_test, y_test,batch_size,model,max_len,WE_model):
        y_pred = np.array([])
